...
class MetricsCalculator:
    ...
    def calculate_nav(self, fund_id: int) -> Optional[Decimal]:
        ...
        Represents the capital still invested in the fund
        """
        total_calls = self.db.query(
            func.sum(CapitalCall.amount)
        ).filter(
        ...
        nav = total_calls - total_distributions - total_adjustments

        return nav if nav > 0 else Decimal(0)

    def calculate_tvpi(self, fund_id: int) -> Optional[float]:
        """
        Calculate TVPI (Total Value to Paid-In)
        TVPI = (Total Distributions + NAV) / PIC
        Measures the total value (realized + unrealized) relative to paid-in capital
        """
        pic = self.calculate_pic(fund_id)
        total_distributions = self.calculate_total_distributions(fund_id)
        nav = self.calculate_nav(fund_id)
    ...
    def get_calculation_breakdown(self, fund_id: int, metric: str) -> Dict[str, Any]:
        ...
        if metric == "dpi":
            ...
            return {
                ...
                "result": float(nav) if nav else 0,
                "explanation": f"NAV = {total_calls} - {total_distributions} - {total_adjustments} = {nav}"
            }
            ...
            total_value = float(total_distributions) + float(nav)
            ...
            return {
                ...
                "pic": float(pic) if pic else 0,
                "result": tvpi,
                "explanation": f"TVPI = ({total_distributions} + {nav}) / {pic} = {tvpi}"
            }
            ...
            return {
                ...
                "pic": float(pic) if pic else 0,
                "result": rvpi,
                "explanation": f"RVPI = {nav} / {pic} = {rvpi}"
            }
