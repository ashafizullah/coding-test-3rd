...
async def export_fund_report(fund_id: int, db: Session = Depends(get_db)):
    """
    Export fund report to Excel format.

    Returns an Excel file with fund information, metrics, and all transactions.
    """
    # Verify fund exists
    fund = db.query(Fund).filter(Fund.id == fund_id).first()
    if not fund:
    ...
    try:
        ...
        excel_data = exporter.export_fund_report(fund_id)

        # Generate filename
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        fund_name_clean = "".join(c for c in fund.name if c.isalnum() or c in (' ', '_')).rstrip()
        fund_name_clean = fund_name_clean.replace(' ', '_')
        filename = f"{fund_name_clean}_Report_{timestamp}.xlsx"
        ...
        logger.error(f"Error exporting fund report: {e}", exc_info=True)
