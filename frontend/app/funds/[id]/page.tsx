...
import { Loader2, TrendingUp, DollarSign, Calendar, BarChart3 } from 'lucide-react'
  ...
  const { data: allTransactions } = useQuery({
    ...
    queryFn: async () => {
      const [calls, dists] = await Promise.all([
        fundApi.getTransactions(fundId, 'capital_calls', 1, 100),
        fundApi.getTransactions(fundId, 'distributions', 1, 100)
      ])

      const transactions = [
        ...(calls?.items || []).map((item: any) => ({
          date: item.call_date,
          type: 'capital_call' as const,
          amount: item.amount
        })),
        ...(dists?.items || []).map((item: any) => ({
      ...
      {allTransactions && allTransactions.length > 0 && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <div className="flex items-center mb-4">
            ...
            <h2 className="text-xl font-semibold">Cash Flow Overview</h2>
          </div>
          <CashFlowChart transactions={allTransactions} chartType="bar" />
        </div>
      )}
